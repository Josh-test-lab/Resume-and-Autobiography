...
目前研究方向為時間序列，利用神經網路處理高維度資料，透過特徵提取（反向傳播與黑箱作業）進行缺值填補，應對機器缺失或其他因素所導致的資料缺失。未來將拓展至空間資料，探索高維度填補方法。  
...
\section*{PROJECT}
\begin{itemize}[left=0pt]
    \item \textbf{志學站停車場停車模擬模型建立專題}（碩士課程）
    ...
    \item \textbf{使用卷積神經網路（CNN）進行影像辨識}（Python）
    \item \textbf{個人網站} \texttt{https://yao-chih.netlify.app/}
\end{itemize}
